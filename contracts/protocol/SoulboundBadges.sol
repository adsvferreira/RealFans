    ...
    ISoulboundBadges
{
    ...
    function mintBadge(
        ...
        require(
            _isBadgeURIWhitelisted(badgeURI),
            "tokenURI noy whitelisted yet"
        );
    ...
    function _isBadgeURIWhitelisted(
    ...
    function getTotalQtyOfBadge(
        string memory badgeURI
    ) public view returns (uint256 totalSupply) {
        totalSupply = _totalBadgeQty[badgeURI];
    }

    function getBadgeQtyOf(
        address account,
        string memory badgeURI
    ) public view returns (uint256 badgeQtyOf) {
