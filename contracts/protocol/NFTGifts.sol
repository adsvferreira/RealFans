...
contract NFTGifts is ERC721, ERC721URIStorage, Ownable, INFTGifts {
    ...
    mapping(string => mapping(string => uint256))
        private _balancesUnclaimedAccounts;
    ...
    function mintGift(
        ...
        string memory donatorTwitterHandle = usersDB
            .getTwitterHandleFromAddress(msg.sender);
        ...
        emit Donation(
            msg.sender,
            to,
            receiverTwitterHandle,
            donatorTwitterHandle,
            giftURI,
            ethValue
        );
    }
    ...
    function redeemDonation(uint256 tokenId) public {
        ...
        string memory receiverTwitterHandle = usersDB
            .getTwitterHandleFromAddress(msg.sender);
    ...
    function redeemDonationsToUnclaimedAccount(string memory giftURI) external {
        ...
        string memory receiverTwitterHandle = usersDB
            .getTwitterHandleFromAddress(to);
        uint256 giftQtyToReceive = _balancesUnclaimedAccounts[
            receiverTwitterHandle
        ][giftURI];
    ...
    function getGiftQtyOfUnclaimedAccount(
    ...
    function getEthBalanceOfPerGiftUnclaimedAccount(
        string memory twitterHandle,
        string memory giftURI
    ) public view returns (uint256 ethBalanceOfPerGift) {
        ethBalanceOfPerGift =
            getGiftQtyOfUnclaimedAccount(twitterHandle, giftURI) *
            getEthValueOfGift(giftURI);
    }
    ...
    function getEthBalanceOfUnclaimedAccount(
        ...
        uint256 giftURIsLength = giftURIs.length;
        for (uint256 i = 0; i < giftURIsLength; ) {
            ethBalanceOf += getEthBalanceOfPerGiftUnclaimedAccount(
                twitterHandle,
                giftURIs[i]
            );
            unchecked {
    ...
    function _updateUnclaimedReceiversList(
        string memory receiverTwitterHandle
    ) private {
        string[] memory giftURIs = _giftURIs;
        uint256 giftURIsLength = giftURIs.length;
        ...
        for (uint256 i = 0; i < giftURIsLength; ) {
            if (
                _balancesUnclaimedAccounts[receiverTwitterHandle][
                    giftURIs[i]
                ] != 0
            ) {
                isFirstDonation = false;
