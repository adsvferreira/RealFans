...
def execute():
    ...
    while True:  # POG
        try:
            ...
            if not to_block > from_block:
                continue
            # Fetch and write user events
            users = contracts["users"]
            user_added_events = [
                event_parser.parse_user_added_event(event)
                for event in users.events.userAdded.createFilter(
            ...
            nft = contracts["nft"]
            nft_donation_events = [
                event_parser.parse_donation_event(event)
                for event in nft.events.Donation.createFilter(fromBlock=from_block, toBlock=to_block).get_all_entries()
            ]
            my_database.add_multiple_entries(my_database.add_donation, nft_donation_events)
            nft_redemption_events = [
                event_parser.parse_redemption_event(event)
                for event in nft.events.Redemption.createFilter(
                    fromBlock=from_block, toBlock=to_block
                ).get_all_entries()
            ]
            ...
            soulbound_mint_events = [
                event_parser.parse_badge_minted_event(event)
                for event in soulbound.events.BadgeMinted.createFilter(
                    fromBlock=from_block, toBlock=to_block
                ).get_all_entries()
            ]
            ...
            print(f"INDEXER EXCEPTION: {type(exc)} => {str(exc)}")
        finally:
            time.sleep(10)
